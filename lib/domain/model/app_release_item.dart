...
class AppReleaseItem implements PlayableItem {
  ...
  ///TO KNOW IF RELEASE WAS UPLOADED FROM USER OR COLLECTIVE TO FLOW
  ...
  ///NAME OF PUBLISHER OR META OWNER
  ...
  ///LYRICS FOR SONGS (plain text from artist)
  String? lyricsLrc;

  ///SYNCED LRC LYRICS (generated by Whisper AI alignment)
  String? language;
  ...
  ///PLACE OR LOCATION FOR METAOWNER IF ABLE.
  ...
  ///Out of the app
  Map<String, String>? featInternalArtists;
  ...
  /// Slug of the artist that owns this release, derived from [ownerName].
  ///
  /// Denormalized so a shareable URL can be built without a second read, and
  /// so it resolves whether or not a Collective exists for the artist yet.
  /// Together with [slug] it forms the `/a/{ownerSlug}/{slug}` address, unique
  /// because an artist cannot have two releases with the same name.
  String ownerSlug;
  ...
  /// Whether this item belongs to the catalogue that may be read without an
  /// account. Draft, pending, private/future, and suspended releases must not
  /// be exposed by guest list or deep-link lookups.
  bool get isPubliclyVisible => status == ReleaseStatus.publish && !isSuspended;

  /// Detached copy containing only the fields needed by a public catalogue or
  /// detail page. Account-scoped engagement and moderation data stays out of
  /// guest controller state even when legacy Firestore rules return the raw
  /// document.
  AppReleaseItem toPublicProjection() {
    final projected = AppReleaseItem.fromJSON(toJSON())..id = id;
    projected.ownerEmail = '';
    projected.metaOwnerId = '';
    projected.boughtUsers = [];
    projected.likedProfiles = [];
    projected.sharedProfiles = [];
    projected.commentIds = [];
    projected.localPath = null;
    projected.state = 0;
    projected.suspendedBy = null;
    projected.suspendedReason = null;
    return projected;
  }
    ...
    : id = data["id"] ?? '',
      ...
      type =
          EnumToString.fromString(
            ReleaseType.values,
            data["type"] ?? ReleaseType.single.name,
          ) ??
          ReleaseType.single,
      status =
          EnumToString.fromString(
            ReleaseStatus.values,
            data["status"] ?? ReleaseStatus.draft.name,
          ) ??
          ReleaseStatus.draft,
      ...
      ownerType =
          EnumToString.fromString(
            OwnerType.values,
            data["ownerType"] ?? OwnerType.notDefined.name,
          ) ??
          OwnerType.notDefined,
      ...
      state = data["state"] ?? 0,
      externalArtists = List.from(
        data["externalArtists"]?.cast<String>() ?? [],
      ),
  ...
  bool get isBookContent {
    // Check by mediaType first
    if (mediaType != null) {
      return mediaType == MediaItemType.book || mediaType == MediaItemType.pdf;
    }
  ...
  /// Slug for an artist name, hyphenated to match `Collective.generateSlug`.
  ///
  /// Deliberately the same shape a Collective would produce, so the artist's
  /// URL does not change the day a band is registered for that name.
  static String generateOwnerSlug(String ownerName) => generateSlug(ownerName);
  ...
  Map<String, dynamic> toJSON() => {
    ...
    'ownerName': ownerName,
    if (ownerProfileId?.isNotEmpty == true) 'ownerProfileId': ownerProfileId,
    ...
    'lyrics': lyrics,
    if (lyricsLrc != null) 'lyricsLrc': lyricsLrc,
    'language': language,
    ...
    if (artCategory != null && artCategory!.isNotEmpty)
      'artCategory': artCategory,
    if (artTechnique != null && artTechnique!.isNotEmpty)
      'artTechnique': artTechnique,
    if (artMedium != null && artMedium!.isNotEmpty) 'artMedium': artMedium,
    if (artDimensions != null && artDimensions!.isNotEmpty)

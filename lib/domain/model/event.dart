...
class Event {
    ...
    : id = data["id"] ?? "",
      ...
      reason =
          EnumToString.fromString(
            UsageReason.values,
            data["reason"] ?? UsageReason.any.name,
          ) ??
          UsageReason.any,
      appMediaItems =
          data["appMediaItems"]?.map<AppMediaItem>((item) {
      ...
      type =
          EnumToString.fromString(
            EventType.values,
            data["type"] ?? EventType.rehearsal.name,
          ) ??
          EventType.rehearsal,
      status =
          EnumToString.fromString(
            EventStatus.values,
            data["status"] ?? EventStatus.draft.name,
          ) ??
          EventStatus.draft,
      ...
      instrumentsFulfillment =
          data["instrumentsFulfillment"]?.map<InstrumentFulfillment>((item) {
            return InstrumentFulfillment.fromJSON(item);
          }).toList() ??
          [],
      collectivesFulfillment =
          data["collectivesFulfillment"]?.map<CollectiveFulfillment>((item) {
            return CollectiveFulfillment.fromJSON(item);
          }).toList() ??
          [],
      watchingProfiles = List.from(
        data["watchingProfiles"]?.cast<String>() ?? [],
      ),
  ...
  Map<String, dynamic> toJSON() => {
    ...
    'appMediaItems': appMediaItems
        ?.map((appMediaItem) => appMediaItem.toJSON())
        .toList(),
    'genres': genres,
    ...
    'instrumentsFulfillment': instrumentsFulfillment
        ?.map((instrumentFulfillment) => instrumentFulfillment.toJSON())
        .toList(),
    'collectivesFulfillment': collectivesFulfillment
        ?.map((collectiveFulfillment) => collectiveFulfillment.toJSON())
        .toList(),

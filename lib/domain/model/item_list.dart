...
class Itemlist {
  ...
  /// Slug of the artist that owns this list, derived from [ownerName].
  ///
  /// Mirrors `AppReleaseItem.ownerSlug` so an album and its tracks address the
  /// same artist: `/album/{ownerSlug}/{slug}` and `/a/{ownerSlug}/{track}`.
  String ownerSlug;
  String name;
  String description;
  String ownerId;
  ...
  /// Hyphenated slug for a list title. Same shape as releases and collectives.
  static String generateSlug(String title) => title
      .toLowerCase()
      .replaceAll(RegExp(r'\s+'), '-')
      .replaceAll(RegExp(r'[^a-z0-9\-áéíóúñü]'), '');

  /// Slug for the owning artist. Identical to the release and collective
  /// generators, so one artist has exactly one address everywhere.

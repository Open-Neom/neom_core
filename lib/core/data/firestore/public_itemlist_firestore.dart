...
class PublicItemlistFirestore { //implements ItemlistRepository {

  var logger = AppUtilities.logger;
  final publicItemlistReference = FirebaseFirestore.instance.collection(AppFirestoreCollectionConstants.publicItemlists);
  ...
  Future<String> insert(Itemlist itemlist) async {
    ...
    try {
      if(itemlist.id.isEmpty) {
        DocumentReference? documentReference = await publicItemlistReference
            .add(itemlist.toJSON());
        itemlistId = documentReference.id;
      } else {
        await publicItemlistReference.doc(itemlist.id).set(itemlist.toJSON());
  ...
  Future<bool> addAppMediaItem(AppMediaItem appMediaItem, String itemlistId) async {
    ...
    try {
       DocumentReference documentReference = publicItemlistReference.doc(itemlistId);
  ...
  Future<Map<String, Itemlist>> fetchAll({bool onlyPublic = false, bool excludeMyFavorites = true, int minItems = 0, int maxLength = 100, String profileId = ''}) async {
    ...
    try {
      await publicItemlistReference.limit(maxLength).get().then((querySnapshot) {
  ...
  Future<bool> remove(itemlistId) async {
    ...
    try {
      ...
      await publicItemlistReference.doc(itemlistId).delete();
  ...
  Future<bool> update(Itemlist itemlist) async {
    ...
    try {
      ...
      DocumentReference documentReference = publicItemlistReference.doc(itemlist.id);
      await documentReference.update({
  ...
  Future<bool> addPreset(String chamberId, ChamberPreset preset) async {
    ...
    try {
      ...
      DocumentReference documentReference = publicItemlistReference.doc(chamberId);
      await documentReference.update({
